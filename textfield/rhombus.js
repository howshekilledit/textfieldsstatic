...
let prism = class {
  ...
  copyTranslated(trans, preload = true) { //returns a copy of the current instance, translated by x and y
    let facets = [];
    for (let f of this.fcts) {
    ...
    let copied = new prism(facets, this.container, this.colors, this.stroke, this.backfill);
    //get attributes from original
    if (preload) { //if preload is true, load all attributes from original
      copied.preload();
      if(this.bg){
        copied.bg.opacity(this.bg.opacity());
      }
      if(this.lines){
  ...
  toggleMask(){
    if(this.masked == false){
      //for each elememnt in bg group
      let mask = this.mask;
      //this.bg.each(function (i, children) {
        //this.fill('#fff');
        this.bg.maskWith(mask);
      //}); 
      //this.bg.maskWith(this.mask.show());
      this.masked = true;
    }
    else{
      //this.bg.fill(this.backfill);
      this.bg.unmask(); 
      this.masked = false;
    }
  }
